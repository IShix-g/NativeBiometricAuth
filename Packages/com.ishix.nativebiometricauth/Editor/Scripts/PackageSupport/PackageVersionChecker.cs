...
{
    public sealed class PackageVersionChecker : IDisposable
    {
        ...
        public void Install(CancellationToken token = default)
            => InstallAsync(token).Handled();
        
        public Task InstallAsync(CancellationToken token = default)
        {
            _tokenSource = CancellationTokenSource.CreateLinkedTokenSource(token);
            return _packageInstaller.Install(new[] {GitInstallUrl}, _tokenSource.Token);
        }
